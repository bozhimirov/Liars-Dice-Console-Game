...
def ask_for_choice(english_language):
...
def text_choose_name(english_language):
...
def text_choose_opponents(english_language):
...
def text_choosing_player(english_language):
...
def text_your_turn_and_info(english_language, sum_dice, players_turn, human_player):
...
def text_new_game_option(english_language):
...
def text_player_bet(language, player_name, current_bet):
...
def text_choose_name_again(language):
...
def text_incorrect_input_opponents(language):
    choose_language(language,
...
def text_left_game(language, inactive_names):
...
def text_someone_call_other_liar(language, current_player, last_player):
